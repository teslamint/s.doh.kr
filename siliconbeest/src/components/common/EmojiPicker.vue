...
<template>
  <div class="sb-menu w-72 max-h-80 flex flex-col overflow-hidden p-0">
    <!-- Search bar -->
    <div class="p-2 border-b border-outline dark:border-outline-dark">
      <input
        v-model="searchQuery"
        type="text"
        :placeholder="t('compose.search_emoji')"
        class="w-full rounded-lg border-0 bg-surface-2 px-2.5 py-1.5 text-sm text-slate-900 transition placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/40 dark:bg-white/5 dark:text-slate-100 dark:placeholder:text-slate-500"
      />
    </div>
    ...
    <div v-if="filteredResults" class="flex-1 overflow-y-auto p-2 min-h-0">
      ...
      <div v-if="filteredResults.custom.length > 0" class="mb-2">
        <div class="px-1 mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">Custom</div>
        <div class="grid grid-cols-8 gap-0.5">
          <button
            ...
            class="w-8 h-8 flex items-center justify-center rounded-lg transition-colors hover:bg-surface-2 dark:hover:bg-white/10"
      ...
      <div v-if="filteredResults.unicode.length > 0">
        <div class="px-1 mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">Emoji</div>
        <div class="grid grid-cols-8 gap-0.5">
          <button
            ...
            class="w-8 h-8 flex items-center justify-center rounded-lg transition-colors hover:bg-surface-2 dark:hover:bg-white/10 text-xl leading-none"
            :title="emoji.name"
          >
            {{ emoji.emoji }}
          </button>
      ...
      <div
        ...
        class="text-sm text-slate-400 dark:text-slate-500 text-center py-6"
      >
    ...
    <template v-else>
      <!-- Category tabs -->
      <div class="flex items-center gap-0.5 px-1.5 py-1 border-b border-outline dark:border-outline-dark overflow-x-auto scrollbar-none">
        <button
          v-for="tab in tabs"
          :key="tab"
          type="button"
          @click="activeTab = tab"
          class="flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-lg text-sm transition-colors"
          :class="activeTab === tab
            ? 'bg-brand-50 text-brand-600 dark:bg-brand-950/60 dark:text-brand-300'
            : 'text-slate-500 hover:bg-surface-2 dark:text-slate-400 dark:hover:bg-white/10'"
          :title="tab"
        >
      ...
      <div class="flex-1 overflow-y-auto p-2 min-h-0" style="max-height: 220px;">
        ...
        <template v-if="customCategoryNames.includes(activeTab)">
          ...
          <div v-if="!customEmojisByCategory.get(activeTab)?.length" class="text-sm text-slate-400 dark:text-slate-500 text-center py-6">
            {{ t('compose.no_custom_emoji') }}
          </div>
        </template>
